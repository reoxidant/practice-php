...
if (isAdmin()) {
    $flag = false;
    if (isset($_REQUEST['type'], $_REQUEST['change'])) {
        $type = $_REQUEST['type'];
        mysqli_query($link, "UPDATE users SET status='$type' WHERE id='$id'") || die (mysqli_error($link));
        $result = mysqli_query($link, "SELECT * FROM users WHERE id='$id'") || die(mysqli_error($link));
        $user = mysqli_fetch_assoc($result);
        $flag = true;
    ...
    <fieldset>
        ...
        <form action="" method="post">
            <p>
                ...
                if ($user['status'] === 10) {
                    ?>
                    <input type="radio" name="type" value="10" checked="checked" id="1"><label
                            for="1">Администратор</label><br>
                    <?php
                } else {
                    ?>
                ...
                if ($user['status'] === 2) {
                    ?>
                    <input type="radio" name="type" value="2" checked="checked" id="2"><label for="2">Модератор</label>
                    <br>
                    <?php
                } else {
                    ?>
                ...
                if ($user['status'] === 1) {
                    ?>
                    <input type="radio" name="type" value="1" checked="checked" id="3"><label
                            for="3">Пользователь</label><br>
                    <?php
                } else {
                    ?>
