...
<body>
<?php
if ($_SESSION['auth'] === true && $_SESSION['id'] === $_REQUEST['id']) {
    ?>
    <fieldset>
        ...
        <p>Ваша дата последнего
            посещения: <?php echo isset($_COOKIE['datetime']) ? $_COOKIE['datetime'] : $user['dt']; ?></p>
        ...
        <?php
        $sql_res = mysqli_query($link, "SELECT * FROM users WHERE status = 2 or status = 10") || die(mysqli_error($link));
        ...
        <p><a href="<?php echo HOST; ?>/tasksAuthAndReg/profile/change_password.php">Сменить пароль</a> или <a
                    href="<?php echo HOST; ?>/tasksAuthAndReg/profile/delete_profile.php">Удалить аккаунт</a></p>
    ...
    <fieldset>
                ...
                if ($_SESSION['id'] !== $row['id']) {
                    ...
                    <tr style="height: 40px;">
                        <td><a href="?id=<?php echo $row['id']; ?>"
                               style="padding-right: 5px;"><?php echo $row['login']; ?></a></td>
        ...
        <div>
            <?php for ($x = 1; $x <= $pages; $x++): ?>
                <a href="?id=<?php echo $id; ?>&page=<?php echo $x; ?>&per-page=<?php echo $perPage; ?>"<?php if ($page === $x) {
                    echo ' class="select"';
                }; ?>><?php echo $x; ?></a>
            <?php endfor; ?>
        </div>
    ...
    if (empty($user) and isset($_REQUEST['id']) or !isset($_REQUEST['id'])) {
        ...
        <fieldset>
            ...
            <?php if (!empty($user)) {
                ?>
                <p>
                    <a href="message.php?id=<?php echo isset($_REQUEST['id']) ? $_REQUEST['id'] : ''; ?>">Отправить
                        личное сообщение</a>
