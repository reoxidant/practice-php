...
if (isset($_REQUEST['hour']) and isset($_REQUEST['days']) and isset($_REQUEST['weeks']) and isset($_REQUEST['ban']) and isset($_REQUEST['user_id'])) {

	$id = $_REQUEST['user_id'];

	$ban = date('Y-m-d H:i:s', strtotime("+ ".$_REQUEST['weeks']." week ".$_REQUEST['days']." days ".$_REQUEST['hour']." hours"));
	 
    mysqli_query($link, "UPDATE users SET banned = '$ban' WHERE id='$id'") or die(mysqli_error($link));
