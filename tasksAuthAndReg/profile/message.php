<?php 
    ...
    if(isset($_REQUEST['message']) and !empty($_REQUEST['message'])){
    	...
    	mysqli_query($link, "INSERT INTO msg SET recipient_id = '$recipient_id', sender_id='$sender_id', message='$msg'") or die (mysqli_error($link));
    }
...
<body>
	<?php 
		if(isset($_REQUEST['id']) and !empty($user) and isset($_SESSION['id'])){
	?>
		<table>
	        <?php
	            ...
	            <tr style="height: 40px;">
	                <td class="<?php echo ($row['sender_id'] == $sender_id)? 'selected': '';  ?>"><?php echo $row['message']; ?>
	                	<?php if($_SESSION['id'] == $row['recipient_id']){
	                		mysqli_query($link, "UPDATE msg SET readed_msg = '1' WHERE recipient_id='$sender_id'") or die(mysqli_error($link));
	                		} ?>
	                </td> 
	            </tr>
	        <?php
	...
	<?php
		}else if(empty($user) or $_SESSION['auth'] = false){
