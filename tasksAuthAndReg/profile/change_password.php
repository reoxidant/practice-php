...
<body>
	...
	<?php
		...
		if(!empty($_REQUEST['old_password']) and !empty($_REQUEST['password_confirm']) and !empty($_REQUEST['new_password']) and isset($_REQUEST['next'])){
			...
			if($new_password == $password_confirm){
				...
				if(checkLenght($new_password, 6, 12)){
					...
					mysqli_query($link, "UPDATE users SET password='$saltedNewPassword' where id='$id'") or die (mysqli_error($link));
